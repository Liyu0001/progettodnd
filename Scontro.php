...
class scontro {
    ...
    function combattimento(  $p1 = new Personaggio,  $p2 = new Personaggio){
        ...
        $scontro = true; 
     //  foreach ($combattenti as $player){
           
        while($scontro) {
        $p1ToHit = $p1->attackRoll();
        if ($p1ToHit == 20){
            $p1Damage = $p1->damageDealt() *2;
            $p2->takeDamage($p1Damage);
            echo  $p1->nome . ' managed a critical hit attack on '. $p2->nome . ' for ' . $p1Damage.'!!!<br>';
        ...
        else if ($p1ToHit > $p2->classeArmatura) {  
        ...
        if($p2ToHit == 20){
            $p2Damage = $p1->damageDealt() *2;
            $p1->takeDamage($p2Damage);
            echo  $p2->nome . ' managed a critical attack on '. $p1->nome . ' for ' . $p2Damage.'!!!<br>';
        ...
        } else if ($p2ToHit > $p1->classeArmatura) {  
